...
def export_frontend_contract_constants(schema: dict[str, Any]) -> Path:
    relation_type_category = {relation_type.value: category.value for relation_type, category in RELATION_TYPE_CATEGORY.items()}
    relation_categories = list(dict.fromkeys(relation_type_category.values()))
    enum_constants = _enum_constants_ts(schema)
    config_constraints = _field_constraints(
        schema,
        ("AgentConfig", "AgentPoolConfig", "AgentRuntimeConfig", "LightRAGConfig"),
    )
    ...
    FRONTEND_CONTRACT_CONSTANTS_PATH.write_text(
        ...
        f"{enum_constants}\n\n"
        f"export const WORK_PROJECT_RELATION_CATEGORY_VALUES = {json.dumps(relation_categories)} as const;\n"
        f"export const WORK_PROJECT_RELATION_CATEGORY_ENUM = {json.dumps(_enum_members('WorkProjectRelationCategory', relation_categories))} as const;\n"
        f"export const WORK_PROJECT_RELATION_CATEGORY = {json.dumps(relation_type_category)} as const;\n"
